...
class LineLabel extends PureComponent {
  ...
  render() {
    ...
    const { classes, x, y, incomeBracket, theme: lineTheme, isVisible } = this.props;

    const padding = 2;
    return (
      <g className={isVisible ? classes.visible : classes.hidden}>
        {rectX && <rect x={rectX - padding} y={rectY - padding} width={width + padding*2} height={height + padding*2} fill={theme[lineTheme]} />}
        <text x={x} y={y} className={classes.text}>Avg. net price for</text>
        <text x={x} y={y + 24} className={classes.text}>
          <tspan className={classes.income} ref={this.incomeRef}>{incomeBrackets[incomeBracket]}</tspan>
